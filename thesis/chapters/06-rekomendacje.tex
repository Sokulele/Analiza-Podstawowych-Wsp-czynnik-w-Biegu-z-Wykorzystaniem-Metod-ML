...
Pipeline kończy się modułem, który przekształca obliczone współczynniki (rozdział~\ref{ch:materialy-metody}) w~konkretne wskazówki treningowe. Rozdział ten opisuje, dlaczego rekomendacje oparłem na regułach z~literatury zamiast na uczeniu maszynowym, jak zbudowany jest silnik reguł, jakie reguły zawiera oraz jak wygląda wygenerowany raport dla rzeczywistego biegacza.

\section{Dlaczego reguły, a~nie uczenie maszynowe}
\label{sec:rule-based-vs-ml}

Rekomendacje nie są uczone z~danych --- są zakodowanymi ręcznie regułami odzwierciedlającymi zakresy referencyjne i~wzorce ryzyka ustalone w~literaturze biomechanicznej. Wybór ten wynika z~czterech przesłanek:
...
Silnik składa się z~11~funkcji reguł, z~których każda analizuje jeden aspekt techniki biegu (np.~kadencję, czas kontaktu, pochylenie tułowia). Każda funkcja otrzymuje na wejściu wyniki obliczeń współczynników (sekcja~\ref{sec:obliczanie-wspolczynnikow}) i~zwraca listę rekomendacji --- obiektów zawierających:

\begin{itemize}
    \item \textbf{Poziom ważności} (\textit{severity}) --- czterostopniowa skala:
    \begin{itemize}
        \item \texttt{critical} --- wartość wymagająca interwencji. Flaga ta zapala się w~dwóch sytuacjach: po pierwsze, gdy wynik wskazuje na rzeczywiste ryzyko biomechaniczne (np.~kąt kolana powyżej $175^\circ$ przy kontakcie = overstriding, kadencja poniżej~150~spm), po drugie, gdy wynik sugeruje, że klasyfikator faz coś źle sklasyfikował i~obliczone współczynniki są niewiarygodne (np.~asymetria liczby kroków lewej i~prawej nogi przekraczająca~20\%, co świadczy o~tym, że model myli fazy). Oba przypadki wymagają reakcji człowieka, ale innego rodzaju: w~pierwszym biegacz powinien skorygować technikę, w~drugim --- należy przenagrać film z~lepszą perspektywą kamery lub w~lepszych warunkach,
    ...
    \end{itemize}
    \item \textbf{Źródło literaturowe} (\textit{citation}) --- autor i~rok publikacji, na której oparta jest reguła,
...
\hline
\textbf{Kategoria} & \textbf{Przykładowa reguła} & \textbf{Źródło} \\
\hline
\multicolumn{3}{l}{\textit{Reguły temporalne}} \\
...
\hline
\multicolumn{3}{l}{\textit{Reguły przestrzenne}} \\
Kolano @~kontakt   & $>175^\circ$ $\to$ critical (overstriding)   & Heiderscheit 2011 \\
...
\hline
\multicolumn{3}{l}{\textit{Reguły walidacyjne}} \\
...
\begin{enumerate}
    \item \textbf{Overstriding (kadencja + GCT)}: kadencja poniżej~160~spm w~połączeniu ze~średnim GCT powyżej~270~ms to klasyczny wzorzec overstridingu --- stopa ląduje zbyt daleko przed środkiem ciężkości \cite{heiderscheit2011}.
    ...
    \item \textbf{Jakość predykcji (SI + confidence)}: maksymalny SI powyżej~50\% w~połączeniu ze~średnią pewnością predykcji poniżej~0{,}90 --- sygnał, że asymetria L/P jest artefaktem klasyfikatora, nie rzeczywistą cechą biegacza. Reguła ta powstała na podstawie analizy przypadku brzegowego jednego z~biegaczy, na których walidowałem cały pipeline (poza zbiorem treningowym, walidacyjnym i~testowym modelu), u~którego pojedyncze wskaźniki jakości (pewność predykcji, liczba kroków L/R) nie sygnalizowały problemu, podczas gdy asymetria GCT wynosiła~60\%.
\end{enumerate}

Reguły łączone powstały z~obserwacji, że pojedyncze wskaźniki bywają zawodne na nietypowych nagraniach. Sprawdzenie dwóch warunków naraz wychwytuje sytuacje, które każdy z~nich osobno by przepuścił.

\section{Źródła literaturowe reguł}
\label{sec:rules-sources}

Zakresy referencyjne i~progi reguł oparłem na sześciu źródłach:
\begin{itemize}
...
\hline
warning & Tułów zbyt pionowy & pochylenie $2{,}3^\circ$ (cel $5$--$15^\circ$) & Novacheck 1998 \\
...
Każda rekomendacja zawiera nie tylko etykietę, ale też zmierzoną wartość, zakres referencyjny, źródło literaturowe oraz --- w~przypadku ostrzeżeń --- konkretną sugestię korygującą (np.~dla zbyt pionowego tułowia: ``ćwicz pochylenie z~kostek, nie z~bioder''). Warto zwrócić uwagę, że ostrzeżenie o~asymetrii (SI~=~18{,}7\%) opatrzone jest w~pełnym raporcie zastrzeżeniem, że część asymetrii w~danych monocularnych 2D może być artefaktem perspektywy --- system nie przedstawia więc swoich wyników jako pewnej diagnozy, lecz jako sygnał do dalszej weryfikacji.
...
\begin{itemize}
    ...
    \item \textbf{Operowanie na wartościach średnich.} Reguły działają na uśrednionych współczynnikach z~całego nagrania, nie na poszczególnych cyklach. Epizodyczne odchylenia (np.~chwilowe zmęczenie pod koniec biegu) mogą zostać uśrednione i~niezauważone.
    \item \textbf{Zależność od jakości współczynników.} Rekomendacje są tak dobre, jak dane wejściowe. Błędy klasyfikatora faz lub artefakty perspektywy (rozdział~\ref{ch:wrazliwosc}) propagują się do rekomendacji --- stąd reguły walidacyjne, które mają wychwytywać najbardziej rażące przypadki.
