...
Tabela~\ref{tab:model-results} zestawia wyniki wszystkich czterech modeli na zbiorze walidacyjnym i~testowym. Wszystkie modele korzystające z~cech inżynierowanych (RF\,v2, oba LSTM) stosowały korektę proporcji obrazu (sekcja~\ref{sec:aspect-fix}). Wartości F1 w~tabeli to makro-uśrednione miary F1 (sekcja~\ref{sec:metryki-f1}). Kolumna ``Różnica'' przedstawia różnicę dokładności między zbiorem walidacyjnym a~testowym (val acc $-$ test acc) i~służy jako miara generalizacji modelu.
...
Tabela~\ref{tab:confusion-lstm} przedstawia macierz pomyłek modelu LSTM\,r1 na zbiorze testowym ($\approx$~1\,450~klatek). Wiersze odpowiadają prawdziwym klasom, kolumny --- klasom predykowanym przez model.
...
Kluczowa obserwacja: modele LSTM radykalnie redukują pomyłki L$\leftrightarrow$R --- z~153 (RF\,v1) do~75 (LSTM\,r1), czyli dwukrotnie. Jest to bezpośrednia konsekwencja kontekstu czasowego: LSTM widzi naprzemienną sekwencję faz (L$\to$F$\to$R$\to$F$\to$L) i~``wie'', że po kontakcie lewej stopy następuje lot, a~potem kontakt prawej. Random Forest, klasyfikujący każdą klatkę niezależnie, nie ma tej informacji.
...
Tabela~\ref{tab:feature-importance} zestawia pięć najważniejszych cech w~obu modelach.
...
Aby ocenić wpływ korekty proporcji obrazu (sekcja~\ref{sec:aspect-fix}), porównałem wyniki modelu LSTM\,r1 przed i~po zastosowaniu korekty. Tabela~\ref{tab:aspect-fix-results} przedstawia dokładność ogólną oraz per-film na zbiorze testowym.
...
Łączna poprawa na zbiorze testowym wyniosła +3{,}8~pp (z~67{,}1\% do~70{,}9\%). Korekta jest prostym krokiem preprocessingu (jedna operacja mnożenia), który nie wydłuża czasu inferencji, a~istotnie poprawia wyniki na nagraniach o~nietypowych proporcjach.
...
\textbf{Film~2} (bieg 13~km/h, niska rozdzielczość 360$\times$450~px) okazał się najtrudniejszy dla wszystkich modeli --- nawet najlepszy LSTM\,r1 osiągnął jedynie 54{,}5\%. Prawdopodobną przyczyną jest niska rozdzielczość, która ogranicza precyzję detekcji keypointów przez MediaPipe, oraz fakt, że biegacz biega w~tempie i~stylu niereprezentowanym w~zbiorze treningowym.
...
Na podstawie przeprowadzonych eksperymentów wybrałem model \textbf{LSTM\,r1 z~korektą proporcji obrazu} jako model główny (\textit{primary model}) do produkcyjnej inferencji w~pipeline'u. Uzasadnienie:

\begin{enumerate}
    \item \textbf{Najwyższa test accuracy} (70{,}9\%) i~F1 (0{,}709) spośród wszystkich testowanych modeli.
    \item \textbf{Najniższy odsetek pomyłek L$\leftrightarrow$R} wśród modeli o~porównywalnej liczbie błędów (75 pomyłek L$\leftrightarrow$R, 17{,}8\%), co przekłada się na wiarygodniejsze współczynniki symetrii.
    \item \textbf{Najlepsza dokładność per-film} na filmie~10 (85{,}9\%), demonstrująca skuteczność korekty proporcji obrazu na nagraniach o~nietypowej orientacji.
\end{enumerate}

Model RF\,v2 (65{,}7\%) stanowi solidny baseline --- jest szybszy w~treningu (sekundy vs minuty) i~nie wymaga GPU, co czyni go użytecznym do szybkiego prototypowania. LSTM\,r2 (68{,}2\%) oferuje nieco niższą dokładność niż LSTM\,r1, ale mniejszy model (187\,779 parametrów) może być preferowany w~zastosowaniach z~ograniczonymi zasobami obliczeniowymi.

Ograniczeniem wszystkich modeli jest niska dokładność na filmie~2 ($\approx$55\%), co wskazuje na potrzebę rozszerzenia zbioru treningowego o~nagrania niskiej rozdzielczości i~zróżnicowanych stylów biegu. Zagadnienie to omawiam szerzej w~rozdziale~\ref{ch:wrazliwosc}.
