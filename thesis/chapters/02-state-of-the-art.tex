...
Zanim przedstawię swoje rozwiązanie, przeprowadziłem przegląd dotychczasowych prac związanych z analizowanym problemem. Obejmuje on cztery obszary: estymację pozy człowieka z nagrań pojedynczej kamery, biomechanikę biegu, klasyfikację faz biegu metodami uczenia maszynowego oraz walidację metod dwuwymiarowych względem trójwymiarowego motion capture. Taki podział pozwala uporządkować najważniejsze zagadnienia potrzebne do uzasadnienia przyjętego podejścia. Na końcu wskazuję luki badawcze, które wypełnia niniejsza praca.


\section{Estymacja pozy z~nagrań pojedynczej kamery}
\label{sec:sota-pose}

Klasyczna analiza ruchu opiera się na systemach motion capture z~markerami (np.~Vicon, Qualisys), w~których ruch rejestruje jednocześnie wiele rozmieszczonych wokół biegacza kamer. Ponieważ każdy znacznik na ciele widzi kilka kamer z~różnych stron, system odtwarza jego położenie w~trzech wymiarach. Systemy te oferują wysoką dokładność, lecz są drogie, wymagają laboratorium i~doświadczonego operatora. Rozwój metod estymacji pozy bez markerów otworzył możliwość analizy ruchu z~nagrania jedną, zwykłą kamerą. W~odróżnieniu od~systemów wielokamerowych daje ona jedynie płaski, dwuwymiarowy obraz, bez bezpośredniej informacji o~głębi --- takie ujęcie określa się w~literaturze jako \textit{monocular}. Frameworki takie jak OpenPose~\cite{cao2021openpose} oraz MediaPipe BlazePose~\cite{bazarevsky2020blazepose}, trenowane m.in.\ na zbiorze COCO~\cite{lin2014coco}, wykrywają keypointy ciała w~każdej klatce takiego nagrania.
...
Cykl biegowy różni się od cyklu chodu, w~biegu występuje faza lotu (obie stopy w~powietrzu), a~zanika faza podwójnego podparcia charakterystyczna dla chodu~\cite{novacheck1998}. Z~tego powodu progi referencyjne i~wzorce ruchu ustalone dla chodu nie przenoszą się wprost na bieg sportowy.

Literatura biomechaniczna dostarcza ugruntowanych współczynników opisujących technikę biegu. Heiderscheit i~wsp.~\cite{heiderscheit2011} wykazali, że manipulacja kadencją wpływa na obciążenia stawów i~wiąże się z~overstridingiem oraz kątem kolana w~momencie kontaktu. Souza~\cite{souza2016} omawia czas kontaktu z~podłożem i~wzorzec lądowania stopy w~kontekście analizy wideo. Novacheck~\cite{novacheck1998} opisuje fazy cyklu biegowego, pochylenie tułowia, kąty stawów i~oscylację pionową. Daoud i~wsp.~\cite{daoud2012} powiązali wzorzec lądowania stopy z~częstością kontuzji u~biegaczy długodystansowych, a~Robinson i~wsp.~\cite{robinson1987} zaproponowali wskaźnik symetrii do ilościowego opisu różnic między stronami ciała. Przeglądy nowsze, jak Figueiredo i~wsp.~\cite{figueiredo2025cadence}, potwierdzają związek kadencji z~zapobiegnięciem kontuzji.
...
W~kontekście biegu klasyczne metody uczenia maszynowego wykorzystywano m.in.\ do wykrywania zmian zmęczeniowych: Dimmick i~wsp.~\cite{dimmick2023fatigue} zastosowali model Random Forest do analizy biomechaniki biegu, a~Martínez-Gramage i~wsp.~\cite{martinez2020rf} --- do redukcji ryzyka kontuzji u~młodych triathlonistów. Prace łączące estymację pozy z~wideo z~klasyfikacją uczenia maszynowego pojawiają się rzadziej. Ali i~wsp.~\cite{ali2024dmd} połączyli estymację pozy (m.in.\ MediaPipe) z~klasyfikatorem SVM oraz LSTM-CNN do rozróżniania patologii chodu. Większość prac dotyczy jednak chodu klinicznego i~opiera się na czujnikach noszonych, nie na czystym wideo.
...
Z~prac tych wyłania się spójny obraz ograniczeń estymacji 2D: błędy dla ruchów poza płaszczyzną obrazu, zależność od perspektywy oraz od pozycji w~polu widzenia. Ograniczenia te są w~literaturze identyfikowane, ale rzadko systematycznie analizowane pod kątem wpływu na konkretne współczynniki --- co stanowi część luki opisanej poniżej.
...
\begin{enumerate}
    \item \textbf{Ukierunkowanie na bieg, nie chód.} Analizowane prace dotyczą niemal wyłącznie chodu, zwykle w~kontekście klinicznym. Bieg sportowy na bieżni ma odmienne fazy (występuje faza lotu), inne progi kadencji i~czasu kontaktu oraz inne wartości referencyjne --- stanowi więc odrębną niszę.
